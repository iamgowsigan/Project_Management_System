<?php
	...
	$sql4 = "SELECT cart.*,bid.*,product_variant.*,cart.p_id,user.* FROM cart 
		LEFT JOIN bid ON bid.p_id=cart.p_id 
		LEFT JOIN product_variant ON product_variant.pv_id=cart.variant LEFT JOIN user ON user.u_id=bid.win_user
		 WHERE cart.book_id=$orderid AND cart.product_type='BIDDING' ORDER by cart.cart_id DESC";
		...
		$sql5 = "SELECT address.*,user.phone as regphone,user.u_id as userid FROM address LEFT JOIN user ON user.u_id=address.u_id WHERE a_id=$getAddress";
										...
										<div class="card">
											<div class="card-body">
												...
												<!-- end row -->
												 ...
												 if($orders[0]['o_type'] == 'BIDDING'){
                                                 
												 	?>
													<h3><span class="badge badge-primary-lighten ">Bidding</span></h3><br>
                                                    <h6>Winner</h6>
													<p><?=$products[0]['name'];?><br><i class="mdi mdi-phone"></i><?=$products[0]['phone'];?></p>
												...
												<div class="row">
													...
													<div class="col-sm-4">
														...
														<address>
															...
															<?=$address[0]['a_city']; ?><br>
															<?=$address[0]['a_country']; ?><br>
                                                           

														</small>
															 <a href="https://maps.google.com/?q=<?=$address[0]['a_map']; ?>" target="_blank"><i class="mdi mdi-map-marker"></i>View Map</a>
														</address>
																	...
																	<tbody>
																		...
																		<tr>
																				...
																				<td>
																					...
																					<div class="media">
																						<img class="mr-3 rounded" src="<?=$imgloc.$products[0]['p_image'];?>" width="70" height="70" onerror="this.src='assets/images/users/avatar-1.jpg'">
																						
																						<div class="media-body">
																							<small><?=$projectcode; ?>P0<?=$p; ?></small><br>
																							<b><?=$products[0]['p_title']; ?></b> <br/>
																							
																							<?php mylan("Variant : ","البديل: "); ?> <?=($products[0]['variant']!=null)? $products[0]['pv_title']:'- - -'; ?>&nbsp;&nbsp;
																							<div style="width:20px;height:20px;background-color:#<?=$products[0]['pv_color'];?>"></div>
																				...
																				<td><?=$products[0]['purchase_price'].' '.$currency; ?></td>
																				<td><?=$products[0]['coupon_price'].' '.$currency; ; ?></td>
																																								
																				<td class="text-right"><b><?=($products[0]['purchase_price']*$products[0]['quantity']).' '.$currency; ?></b></td>
																			</tr>
...
الرسوم المقتبسة المتفق عليها المذكورة أعلاه."); ?>
													...
													<div class="col-6">
														<div class="mt-3 mt-sm-0">
															...
															<div class='row'>
																<div class='col-8'><b class="float-right"><?php mylan("Total Items : ","مجموع العناصر:"); ?></b></div>

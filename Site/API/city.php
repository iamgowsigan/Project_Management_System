...
if (strlen($_SESSION['login']) == 0) {
	...
	if (isset($_POST['update'])) {
		...
		$res = Updatedata("city", $field, "c_id =$eid");
		if ($res != 0) {
	...
	if ($_GET['action'] == 'del' && $_GET['scid']) {
		$id = intval($_GET['scid']);
		$res = Deletedata("delete from city where c_id='$id'");
	...
	if ($_GET['action'] == 'status' && $_GET['scid']) {
		...
		$val = $_GET['val'];
		$field['b_active'] = $val;
		$res = Updatedata("city", $field, "c_id=$scid");
		$msg = "Success ";
		//header("location: ".$_SERVER['PHP_SELF']);
	}
						...
						<!-- container START-->
										...
										<div class="card-body">
											<table data-order='[[ 0, "desc" ]]' id="basic-datatable"
												...
												<tbody>
													...
													$listdata = Selectdata("Select * FROM city");
													...
													if (sizeof($listdata) == 0) {
														...
														foreach ($listdata as $row) {
															...
															<tr>
																...
																<td>
																	<?php text($row['c_name'], $strong = true, $small = false, $badge = false, $lighten = false, $outline = false, ''); ?>
																</td>

																<td>
																	<?php action($row['c_id'], 'E,D', $name = $row['c_name']); ?>
																</td>
							...
							<?php if ($edit == 1) { ?>
								...
								<div class="col-lg-12">
									...
									<?php
									$listdata = Selectdata("Select * FROM city WHERE c_id=$eid");
									foreach ($listdata as $row) {
										?>
												...
												<div class="card">
													<div class="card-body">
														...
														<div class="row">
															...
															<?php forminput('Name', 'c_name', $row['c_name'], 'required', '6', 'text'); ?>

...
if (strlen($_SESSION['login']) == 0) {
	...
	mysqli_query($con, "UPDATE inspection SET adminview='1' WHERE adminview=0");

	//ADD
	if (isset($_POST['submit'])) {

		$file_name = Uploadimage('inspection', 'image');

		$field['b_id'] = $bid;
		$field['s_id'] = $sid;
		$field['u_id'] = addslashes($_POST['u_id']);
		$field['i_checklist'] = addslashes(implode(',', $_POST['i_checklist']));
		$field['i_remark'] = addslashes($_POST['i_remark']);
		$field['i_cost'] = addslashes($_POST['i_cost']);
		$field['i_lat'] = addslashes($_POST['i_lat']);
		$field['i_lng'] = addslashes($_POST['i_lng']);
		$field['i_dated'] = addslashes($_POST['i_dated']);
		$field['i_image'] = $file_name;

		$res = Insertdata("inspection", $field);
		if ($res != 0) {
			$msg = "Success ";
	...
	//	$field['s_id'] = $sid;
		...
		$res = Updatedata("inspection", $field, "i_id =$eid");
		if ($res != 0) {
	...
	if ($_GET['action'] == 'del' && $_GET['scid']) {
		$id = intval($_GET['scid']);
		$res = Deletedata("delete from inspection where i_id='$id'");
	...
	if ($_GET['action'] == 'update' && $_GET['scid']) {
		...
		$val = $_GET['val'];
		$field['i_status'] = $val;
		$res = Updatedata("inspection", $field, "i_id=$scid");


		$getSite = Selectdata("SELECT * FROM inspection WHERE i_id='$scid'");
		$siteid=$getSite[0]['s_id'];
		$siteservicedate=$getSite[0]['i_dated'];
		$date_plus = date('Y-m-d', strtotime($siteservicedate.'+50 days'));

		$field2['s_servicedate'] = $date_plus;
		$res = Updatedata("sites", $field2, "s_id=$siteid");


		$msg = "Success. Next service date is ".$date_plus;
		//	header("location: ".$_SERVER['PHP_SELF']);
	}
				...
				<div class="content">
					...
					<div class="container-fluid">
						...
						<!-- container START-->
										...
										<div class="card-body">
											<table data-order='[[ 0, "desc" ]]' id="basic-datatable"
												...
												<tbody>
													...
													if ($getpage == "user") {
														$userclass = 'WHERE inspection.s_id =' . $sid;
													}

													$listdata = Selectdata("SELECT inspection.*,bank.*,user.*,sites.* FROM inspection 
													JOIN bank ON bank.b_id=inspection.b_id 
													JOIN sites ON sites.s_id=inspection.s_id 
													JOIN user ON user.u_id=inspection.u_id $userclass");
													...
													if (sizeof($listdata) == 0) {
														...
														foreach ($listdata as $row) {
															...
															<tr>
																...
																<td>
																	<?php text($row['i_id'], $strong = false, $small = false, $badge = false, $lighten = false, $outline = false, ''); ?>
																</td>



																<td><a href="<?= $imgloc . $row['i_image']; ?>" target="blank">
																		<?php image($row['i_image'], '70', '90', 'cover'); ?></a>
																</td>
																...
																<td>
																	...
																	<div class="media">
																		<img class="mr-2 rounded-circle"
																			src="<?= $imgloc . $row['profile_pic']; ?>" width="40" height="40">
																...
																<td>
																	<?php text('Rs ' . $row['i_cost'], $strong = true, $small = false, $badge = false, $lighten = false, $outline = false, 'success', ); ?>
																</td>

																<td>
																	<?php text($row['i_dated'], $strong = false, $small = true, $badge = false, $lighten = false, $outline = false, '', ); ?>
																</td>

																<td>
																	<?php statusactive($row['i_id'], $row['i_status'], 'A,P,C'); ?>
																</td>

																<td>
																	<?php action($row['i_id'], 'E,D', $name = $row['s_name']); ?>
																</td>
												...
												<div class="card">
													<div class="card-body">
														...
														<div class="row">
															...
															<?php forminput('Service Date', 'i_dated', $row['i_dated'], 'required', '6', 'datetime-local'); ?>
															
															<?php formtextarea('Remark', 'i_remark', $row['i_remark'], 'required', '12', 'text'); ?>
															...
															<div class="col-6">
																<p>Record Location<br>
																	<?= $row['i_lat']; ?>,
																	<?= $row['i_lng']; ?><br>
																	<a href="https://www.google.com/maps/search/?api=1&query=<?= $row['i_lat']; ?>,<?= $row['i_lng']; ?>"
							...
							<?php forminput('Service Date', 'i_dated', '', 'required', '6', 'datetime-local'); ?>

							<?php forminput('Cost', 'i_cost', '', 'required', '6', 'text'); ?>
							
							<?php forminput('Remark', 'i_remark', '', 'required', '12', 'text'); ?>

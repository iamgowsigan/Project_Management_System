<?php
	...
	if(strcmp($AppKey,$key)==0){
		...
		mysqli_query($conn,"UPDATE publication SET p_view=p_view+1 WHERE p_id=$id");
		...
		$sql = "SELECT $publicationField FROM publication WHERE p_active=1 AND complete=1 AND p_title$lang!='' AND base='$id' ORDER BY p_date DESC LIMIT 5";
		$result1 = $conn->query($sql);	
		if ($result1->num_rows >0) 
		{
			while($row = $result1->fetch_assoc()) 
			{	
				array_push($otherlang,$row);
			}
		} 
		...
		$sqll="SELECT * from library WHERE u_id=$uid AND f_id=$id AND screen='$screen'";
		...
		$queryBuyer = mysqli_query($conn, "SELECT * from buyer WHERE u_id=$uid AND f_id=$id AND screen='$screen'");
		$countBuyer = mysqli_num_rows($queryBuyer);
		...
		echo json_encode(['success' => true, 'post' => $post,'media' => $media,'reviews'=>$reviews,'likes'=>$countLike,'library'=>$countLibrary ,'buyer'=>$countBuyer,'otherlang'=>$otherlang,'sqll'=>$sqll ]);
		...
		echo json_encode(['success' => false, 'post' => $post,'media' => $media, 'reviews'=>$reviews,'likes'=>$countLike,'library'=>$countLibrary ,'buyer'=>$countBuyer,'otherlang'=>$otherlang ]);
